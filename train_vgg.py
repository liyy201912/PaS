...
# from cusvgg import get_RepVGG_func_by_name
from mcunet import get_RepVGG_func_by_name
from prunesearch import append_loss
...
def parse():
    ...
    parser.add_argument('--momentum', default=0.875, type=float, metavar='M',
                        help='momentum')
    parser.add_argument('--weight-decay', '--wd', default=3.052e-5, type=float,
...
def main():
    ...
    lr_scheduler = CosineAnnealingLR(optimizer=optimizer,
                                     T_max=args.epochs * IMAGENET_TRAINSET_SIZE // args.batch_size // 8)
    ...
    #     ADMM = admm.ADMM(model, file_name=args.prune_setting + ".yaml", rho=args.rho)
    #     if args.admm:
    ...
    #     print('start new admm prune from this sparsity')
        ...
        if args.freeze:
            for name, params in model.named_parameters():
                if 'scale' in name:
                    ...
                    print('epoch and freezed params: ', epoch, name)
        if args.distributed:
...
def train(train_loader, model, criterion, optimizer, epoch, args, lr_scheduler):
    ...
    model.train()
    if args.local_rank == 0:
        print('current learning rate: ', lr_scheduler.get_lr()[0])
    end = time.time()
    ...
    while input is not None:
        ...
        if args.regularization:
            reg_loss = append_loss(model, epoch, args.prune)
        else:
        ...
        # admm update
        # if args.admm:
        #     loss, admm_loss, mixed_loss = admm.append_admm_loss(args, ADMM,
        #                                                         model, loss)  # append admm losss
        #
        ...
        if i % args.print_freq == 0:
            ...
            # if args.admm and args.local_rank == 0:
            #     print('current admm loss: ', mixed_loss.item() - loss.item())
            # if i % (args.print_freq * 40) == 0 and args.admm:
            #     ADMM.adjust_ratio(0.01, writer, i // (args.print_freq * 40) + (epoch-60) * 3)
            # if args.masked_retrain and args.local_rank == 0:
            #     test_bn_sparsity(model)

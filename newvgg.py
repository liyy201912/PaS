...
class RepVGGBlock(nn.Module):
    ...
    def forward(self, inputs):
        if hasattr(self, 'rbr_reparam'):
            return self.scale(self.nonlinearity(self.rbr_reparam(inputs)))
...
def create_model_from_checkpoint(architecture, checkpoint, deploy=False):
    ...
    #     if 'scale' in name:
    #         y = (params > 0.5).float()
    #         print(name, 'remained: ', (torch.sum(y) / y.size(0)).item())

    aabb = RepVGG(num_blocks=[4, 6, 16, 1], num_classes=1000,
                  width_multiplier=[2, 2, 2, 4], override_groups_map=None, deploy=deploy)
